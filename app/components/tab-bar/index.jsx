...
const Title = props => {
  ...
  return (
    ...
    <div className={classname(styles['tab-bar-title'], { active })} onClick={() => onClick(info.index)}>
      {info.title}
    </div>
  )
}
  ...
  return (
    <div>
      <div className={styles['tab-bar-top']}>
        {tabs.map(item => (
          <Title info={item} key={item.index} active={item.index === active} onClick={index => setActive(index)} />
        ))}
      </div>
